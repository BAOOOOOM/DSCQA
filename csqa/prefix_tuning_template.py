################Replace to:csqa/openprompt/prompts/##################
from functools import partial
from transformers.configuration_utils import PretrainedConfig
from transformers.models.gpt2.configuration_gpt2 import GPT2Config
from ..plms.modeling_t5 import T5Config,T5ForConditionalGeneration
...
class PrefixTuningTemplate(Template):
    ...
    def __init__(self,
        ...
        self.sum_num_token=num_token+1
    ...
    def get_past_key_values(self, batch_size=1, mulit_prefix=None):
        pvs = []
        if self.config.is_encoder_decoder and self.using_encoder_past_key_values:
            ...
            if mulit_prefix!=None:
                mulit_past_key_values = self.dynamic_control_trans(mulit_prefix) #bsz, seqlen, layer*emb
                _, mulit_seqlen, _ = mulit_past_key_values.shape
                mulit_past_key_values = mulit_past_key_values.view(batch_size, mulit_seqlen, self.match_n_layer * 2, self.match_n_head,
                                                    self.match_n_embd)
                mulit_past_key_values = self.dropout(mulit_past_key_values)
                if self.num_token!=0:
                    past_key_values=torch.cat((past_key_values,mulit_past_key_values),dim=1)
                else:
                    past_key_values=mulit_past_key_values
            past_key_values = past_key_values.permute([2, 0, 3, 1, 4]).split(2)#self.match_n_layer * 2, batch_size, self.match_n_head, mulit_seqlen, self.match_n_embd
        ...
        if (not self.config.is_encoder_decoder) or self.using_decoder_past_key_values:
            ...
            if mulit_prefix!=None:
                mulit_decoder_past_key_values = self.dynamic_decoder_control_trans(mulit_prefix) #bsz, seqlen, layer*emb
                _, mulit_decoder_seqlen, _ = mulit_decoder_past_key_values.shape
                mulit_decoder_past_key_values = mulit_decoder_past_key_values.view(batch_size, mulit_decoder_seqlen, self.match_n_decoder_layer * 2, self.match_n_head,
                                                    self.match_n_embd)
                mulit_decoder_past_key_values = self.dropout(mulit_decoder_past_key_values)
                if self.num_token!=0:
                    decoder_past_key_values=torch.cat((decoder_past_key_values,mulit_decoder_past_key_values),dim=1)
                else:
                    decoder_past_key_values=mulit_decoder_past_key_values
    ...
    def generate_parameters(self) -> None:
        ...
        if self.config.is_encoder_decoder and self.using_encoder_past_key_values:
            ...
            self.dynamic_control_trans = nn.Sequential(
                nn.Linear(768, self.mid_dim),
                nn.Tanh(),
                # nn.Linear(self.mid_dim, self.mid_dim),
                # nn.Tanh(),
                nn.Linear(self.mid_dim, self.n_layer * 2 * self.n_embd))
        ...
        if (not self.config.is_encoder_decoder) or self.using_decoder_past_key_values:
            ...
            self.dynamic_decoder_control_trans = nn.Sequential(
                nn.Linear(768, self.mid_dim),
                nn.Tanh(),
                # nn.Linear(self.mid_dim, self.mid_dim),
                # nn.Tanh(),
    ...
    def process_batch(self, batch: Union[Dict, InputFeatures], mulit_prefix=None) -> Union[Dict, InputFeatures]:
        r"""
        Convert input_ids to inputs_embeds
        for normal token, use the embedding inside PLM
        for new token, use MLP or LSTM
        """
        if batch['input_ids']!=None:
            batch_size = batch['input_ids'].size(0)
        else:
            batch_size = batch['inputs_embeds'].size(0)
        self.past_key_values = self.get_past_key_values(batch_size=batch_size,mulit_prefix=mulit_prefix)
        ...
        if self.config.is_encoder_decoder:
            ...
            past_key_values=self.past_key_values[1]
            if 'attention_mask' in batch:
                am = batch['attention_mask']
                batch['attention_mask'] = torch.cat([torch.ones((batch_size,self.sum_num_token), dtype = am.dtype,device=am.device), am], dim=-1)
                ...
                for i, layer_module in enumerate(model.encoder.block):
                    ...
                    def modified_encoder_forward(*args, **kwargs):
                        ...
                        if kwargs['attention_mask'] is not None:
                            ...
                            kwargs['attention_mask'] = torch.cat([-torch.zeros((*am.shape[:-1],self.sum_num_token), dtype = am.dtype,device=am.device), am], dim=-1)
                ...
                for i, layer_module in enumerate(model.decoder.block):
                    ...
                    def modified_decoder_self_attn_forward(*args, **kwargs):
                        ...
                        if kwargs['past_key_value'][0].size(-2) + args[0].size(-2) == kwargs['attention_mask'].size(-1):
                            pass
                        elif kwargs['past_key_value'][0].size(-2) + args[0].size(-2) == kwargs['attention_mask'].size(-1) +self.sum_num_token:
