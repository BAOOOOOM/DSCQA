...
def load_dataset(path, config):
    ...
    with open(path, 'r', encoding='UTF-8') as f:
        for line in tqdm(f):
            ...
            #query="Judge the correctness of the sentence: "+question
            query=question
            label=line_dict["question_class"]
            #因为t5模型的说明是要生成文本，因此即使是数字形式也要转换为字符，从而不进行修改
            #confidence = line_dict["confidence"]
            raw_sent = SEP.join([query])
            contents.append([raw_sent, id, label])
    return contents
